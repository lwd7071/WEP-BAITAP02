...
<main class="page-shell">
    <div class="page-heading">
        <div><div class="eyebrow">CATEGORY CỦA TÔI</div><h1>Danh mục của tôi</h1><p>Chỉ hiển thị danh mục thuộc tài khoản đang đăng nhập.</p></div>
        <a class="button primary inline" href="${pageContext.request.contextPath}/category/add">+ Thêm danh mục</a>
    </div>
    ...
    <form class="search-bar" method="get" action="${pageContext.request.contextPath}/categories">
        ...
        <c:if test="${not empty keyword}"><a href="${pageContext.request.contextPath}/categories">Xóa lọc</a></c:if>
    </form>
        ...
        <table>
            ...
            <c:forEach items="${categories}" var="category" varStatus="loop">
                ...
                <tr>
                    ...
                    <td class="actions">
                        <a href="${pageContext.request.contextPath}/category/edit?id=${category.categoryId}">Sửa</a>
                        <a class="delete" onclick="return confirm('Xóa danh mục này?')" href="${pageContext.request.contextPath}/category/delete?id=${category.categoryId}">Xóa</a>
                    </td>
                </tr>
            </c:forEach>
    ...
    <nav class="pagination">
        <c:forEach begin="0" end="${totalPages - 1}" var="index">
            <c:url var="pageUrl" value="/categories"><c:param name="page" value="${index}"/><c:if test="${not empty keyword}"><c:param name="q" value="${keyword}"/></c:if></c:url>
