...
<main class="auth-shell compact">
    ...
    <section class="auth-card wide">
        ...
        <form action="${pageContext.request.contextPath}/register" method="post" class="form-grid">
            <label>Họ và tên<input name="fullname" required value="${fn:escapeXml(fullname)}" placeholder="Nguyễn Văn A"></label>
            <label>Tài khoản<input name="username" required minlength="3" value="${fn:escapeXml(username)}" placeholder="nguyenvana"></label>
            <label>Email<input type="email" name="email" required value="${fn:escapeXml(email)}" placeholder="name@example.com"></label>
            <label>Số điện thoại<input name="phone" value="${fn:escapeXml(phone)}" placeholder="09xxxxxxxx"></label>
