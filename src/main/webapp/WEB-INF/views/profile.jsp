                    ...
                    <main class="page-shell narrow">
                        ...
                        <div class="profile-card">
                            <div class="profile-avatar-section">
                                <c:choose>
                                    <c:when
                                        test="${fn:startsWith(user.avatar, 'http://') or fn:startsWith(user.avatar, 'https://')}">
                                        <c:set var="avatarSrc" value="${user.avatar}" />
                                    </c:when>
                                    <c:when test="${not empty user.avatar}">
                                        <c:url var="avatarSrc" value="/image">
                                            <c:param name="fname" value="${user.avatar}" />
                                        </c:url>
                                    </c:when>
                                    <c:otherwise>
                                        <c:set var="avatarSrc"
                                            value="${pageContext.request.contextPath}/assets/default-category.svg" />
                                    </c:otherwise>
                                </c:choose>
                                <img class="profile-avatar-preview" src="${fn:escapeXml(avatarSrc)}"
                                    alt="Avatar của ${fn:escapeXml(user.fullName)}">
                            ...
                            <form class="editor-card form-stack" method="post" enctype="multipart/form-data"
                                ...
                                <div class="field-grid">
                                    ...
                                    <label>Email
                                        <input type="email" value="${fn:escapeXml(user.email)}" readonly
                                            class="input-readonly">
                                    </label>
                                </div>
                                ...
                                <div class="avatar-inputs">
                                    <label>Tải lên ảnh mới
                                        ...
                                        <small>Chấp nhận JPG, PNG, GIF, WEBP dưới 5MB. Ưu tiên cao hơn link
                                            online.</small>
                                    </label>
